...
public class EvidenceService {
  ...
  public EvidenceService(EvidenceRepository evidence, StoredFileRepository files, FileStorageService storage, ControlRepository controls, RemediationTaskRepository tasks) {
    this.evidence = evidence; this.files = files; this.storage = storage; this.controls = controls; this.tasks = tasks;
  }
  @Transactional
  ...
  @Transactional
  public EvidenceResponse status(UUID orgId, Role role, UUID id, EvidenceStatus status) {
    RoleGuard.requireContributor(role);
    if (status == null) throw ApiException.badRequest("status is required");
    var e = entity(orgId, id); e.status = status; return toDto(e);
  }
  @Transactional
