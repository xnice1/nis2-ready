...
public class OrganizationService {
  ...
  public OrganizationService(OrganizationRepository organizations, MembershipRepository memberships, UserRepository users, AuditService audit) {
    this.organizations = organizations; this.memberships = memberships; this.users = users; this.audit = audit;
  }
  ...
  @Transactional
  public OrganizationResponse update(UUID orgId, UUID actorUserId, Role actorRole, UpdateOrganizationRequest request) {
    ...
    audit.record(orgId, actorUserId, AuditEventType.ORGANIZATION_UPDATED, AuditOutcome.SUCCESS, "ORGANIZATION", orgId, "Organization settings updated",
      java.util.Map.of("organizationName", org.name, "actorRole", actorRole.name()));
    return toDto(org);
  }
  ...
  @Transactional
  public MemberResponse invite(UUID orgId, UUID actorUserId, Role actorRole, InviteMemberRequest request) {
    RoleGuard.requireOrganizationManager(actorRole);
    ...
    audit.record(orgId, actorUserId, AuditEventType.MEMBER_INVITED, AuditOutcome.SUCCESS, "USER", user.id, "Organization member invited",
      java.util.Map.of("email", user.email, "role", membership.role.name(), "actorRole", actorRole.name()));
